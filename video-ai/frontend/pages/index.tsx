...
function isPlayableUrl(url: string): boolean {
  try {
    const u = new URL(url)
    const exts = ['.mp4', '.webm', '.mov', '.m4v']
    return exts.some(e => u.pathname.toLowerCase().endsWith(e)) ||
      ['drive.google.com', 'dropbox.com', 'storage.googleapis.com', 'cloudinary.com', 'blob.vercel-storage.com'].some(h => u.hostname.includes(h))
  } catch { return false }
}
...
export default function VideoAI() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setProgressStep(s => Math.min(s + 1, PROCESS_STEPS.length - 1)), 500 + Math.random() * 800)
    return () => clearTimeout(t)
  }, [processing, progressStep])

  const captureThumbnail = useCallback((url: string) => {
    const el = document.createElement('video')
    el.crossOrigin = 'anonymous'; el.src = url; el.muted = true; el.currentTime = 1
    el.onloadeddata = () => {
      const c = document.createElement('canvas')
      c.width = el.videoWidth || 640; c.height = el.videoHeight || 360
      c.getContext('2d')?.drawImage(el, 0, 0, c.width, c.height)
      setThumbDataUrl(c.toDataURL('image/jpeg', 0.7)); el.remove()
    }
    el.onerror = () => { setThumbDataUrl(null); el.remove() }
  }, [])

  const submitVideo = async () => {
    setLoading(true); setProgressStep(0); setThumbDataUrl(null); setVideoReady(false); setVideoError(null)
    try {
      ...
      if (inputMode === 'url') {
        if (!videoUrl || !isValidUrl(videoUrl)) { setLoading(false); return }
        submitUrl = convertToDirectUrl(videoUrl)
        playUrl = submitUrl
        vidId = `url_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`
        setVideoPlayUrl(playUrl)
        if (!extractYouTubeId(playUrl)) captureThumbnail(playUrl)
      } else {
        if (!video) { setLoading(false); return }
        playUrl = URL.createObjectURL(video)
        submitUrl = `local://${video.name}`
        vidId = `file_${video.name.replace(/\s/g, '_')}_${video.size}_${Date.now()}`
        setVideoPlayUrl(playUrl)
        captureThumbnail(playUrl)
      }

      setVideoId(vidId)
      await axios.post('/api/upload', { url: submitUrl, filename: video?.name || submitUrl.split('/').pop() })
      setProcessing(true)
      const proc = await axios.post(`/api/process/${vidId}`)
      setData(proc.data.sample_data)
      setProcessing(false)
    } catch (err: any) {
      showToast('error', err.response?.data?.error || err.message || 'Processing failed')
    }
  ...
  const seekTo = (time: number) => {
    if (playerRef.current && !videoError) playerRef.current.currentTime = time
  }
  ...
  const isValidUrl = (u: string) => { try { return ['http:', 'https:'].includes(new URL(u).protocol) } catch { return false } }
  ...
  const isLocalBlob = videoPlayUrl?.startsWith('blob:')
  ...
  const renderToast = () => {
    ...
    return (
      <div className={`fixed top-6 right-6 z-50 px-5 py-3 rounded-xl border backdrop-blur shadow-2xl animate-slide-in ${colors[toast.type]}`}>
  ...
  const renderTimeline = () => {
    ...
    const dur = data.duration; const curPct = dur > 0 ? (currentTime / dur) * 100 : 0
    return (
      <div ref={timelineRef} className="relative h-10 mt-2 mb-1 select-none">
        <div className="absolute inset-0 bg-white/[0.05] rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-purple-600/30 to-pink-600/30 transition-all duration-300 rounded-full" style={{ width: `${curPct}%` }} />
        </div>
        {timelineMarkers.map((m, i) => (
          <button key={i} onClick={() => seekTo(m.time)}
            className={`absolute top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full transition-all duration-200 hover:scale-150 hover:z-10 ${Math.abs(currentTime - m.time) < 3 ? 'ring-2 ring-white/50 scale-125' : ''}`}
            style={{ left: `${(m.time / dur) * 100}%`, backgroundColor: m.color }} title={`${formatTime(m.time)} - ${m.label}`} />
        ))}
        <div className="absolute top-full left-0 right-0 flex justify-between text-[10px] text-gray-600 mt-0.5"><span>0:00</span><span>{formatTime(dur)}</span></div>
      </div>
    )
  }
      ...
      <div className="bg-black rounded-2xl overflow-hidden border border-white/[0.07] shadow-2xl">
        ...
        {videoError && (
          <div className="aspect-video bg-gray-900 flex flex-col items-center justify-center gap-3 p-8">
            ...
            <p className="text-gray-500 text-xs text-center break-all max-w-md">{videoPlayUrl}</p>
          </div>
        )}
      </div>
    )
    ...
    <div className={`border-2 border-dashed rounded-xl p-12 text-center cursor-pointer transition-all duration-300 ${
      ...
      {video ? (
        ...
        <div><p className={`text-4xl mb-4 transition-transform duration-300 ${hoverDrop ? 'scale-110' : ''}`}>&#127916;</p>
          <p className="text-gray-300">Drop video here or click to browse</p><p className="text-sm text-gray-500 mt-2">MP4, MOV, WEBM</p></div>
      )}
      <input ref={fileRef} type="file" accept=".mp4,.mov,.webm,.mkv,.avi" className="hidden"
        onChange={e => { const f = e.target.files?.[0]; if (f && f.size > 500 * 1024 * 1024) { showToast('error', 'Max 500MB'); return }; setVideo(f || null) }} />
    </div>
  )
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="relative max-w-7xl mx-auto p-6">
        <header className="text-center mb-10 pt-8">
          ...
          <p className="text-purple-200/50 text-lg">Upload, transcribe, search and ask about any video</p>
        </header>
        ...
        {!data && !processing && (
          <div className="max-w-xl mx-auto bg-white/[0.04] backdrop-blur-2xl rounded-2xl border border-white/[0.07] shadow-2xl p-8">
            ...
            {inputMode === 'url' ? (
              <div className="space-y-3">
                ...
                {videoUrl && isValidUrl(videoUrl) && (
                  <div className="text-xs">{extractYouTubeId(videoUrl) ? <span className="text-green-400">&#9679; YouTube URL</span> : <span className="text-yellow-400">&#9679; External URL</span>}</div>
                )}
              </div>
            ) : (
              renderDrop()
            ...
            <button onClick={submitVideo} disabled={!((inputMode === 'url' ? (videoUrl && isValidUrl(videoUrl)) : video)) || loading}
              ...
              {loading ? 'Processing...' : inputMode === 'file' ? 'Process Local Video' : 'Analyze Video'}
            </button>
          </div>
        )}
                ...
                <div className="flex overflow-x-auto border-b border-white/[0.07]">
                  {(['summary', 'chapters', 'moments', 'transcript', 'qa'] as Tab[]).map(tab => (
                    <button key={tab} onClick={() => setActiveTab(tab)}
                      ...
                      {tab === 'qa' ? 'Q&A' : tab}{tab === 'transcript' && searchTerm ? <span className="ml-1.5 text-emerald-400">({searchMatches.length})</span> : ''}
                    </button>
                  ))}
                  <div className="ml-auto flex gap-1 px-2 items-center">
                    <button onClick={() => downloadAs('json')} className="p-2 text-gray-500 hover:text-gray-300" title="Download JSON">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                    </button>
                    <button onClick={() => downloadAs('md')} className="p-2 text-gray-500 hover:text-gray-300" title="Download Markdown"><span className="text-[10px] font-bold">MD</span></button>
                  </div>
                </div>
                <div className="p-6">
                  {activeTab === 'summary' && (
                    <div className="space-y-4">
                      ...
                      <div className="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06]"><p className="text-gray-200 leading-relaxed">{data.summary}</p></div>
                    </div>
                  )}
                  {activeTab === 'chapters' && (
                    <div className="space-y-2">
                      {data.chapters?.map((ch: any, i: number) => (
                        <button key={i} onClick={() => seekTo(ch.start)}
                          className="w-full flex items-center gap-4 p-3 bg-white/[0.03] hover:bg-white/[0.06] rounded-xl text-left group">
                          ...
                          <div className="flex-1 min-w-0"><span className="text-gray-200 block truncate group-hover:text-purple-300">{ch.title}</span>{ch.confidence ? <span className="text-[11px] text-gray-500">confidence: {(ch.confidence * 100).toFixed(0)}%</span> : ''}</div>
                  ...
                  {activeTab === 'moments' && (
                    <div className="space-y-2">
                      {data.key_moments?.map((m: any, i: number) => (
                        <button key={i} onClick={() => seekTo(m.time)}
                          className="w-full flex items-start gap-4 p-3 bg-white/[0.03] hover:bg-white/[0.06] rounded-xl text-left group">
                          ...
                          <span className="text-gray-300 group-hover:text-gray-200">{m.text.slice(0, 150)}</span>
                        </button>
                      ))}
                    </div>
                  )}
                    ...
                    <div className="space-y-4">
                      ...
                      {qaResult ? (
                        <div className="p-4 bg-white/[0.04] rounded-xl border border-white/[0.07]">
                          <div className="flex items-center gap-2 mb-2"><span className="text-[11px] px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/20">Answer</span></div>
                        ...
                        <div className="space-y-2">
                          ...
                          {qaSources.map((s: any, i: number) => (
                            <button key={i} onClick={() => seekTo(s.time)}
                              ...
                              {s.relevance ? <span className="text-[11px] text-gray-500 shrink-0 mt-0.5">{(s.relevance * 100).toFixed(0)}%</span> : ''}
                            </button>
                          ))}
                        </div>
                      ) : ''}
            ...
            <div className="xl:col-span-2 space-y-3">
              {data.tech_insights ? (
                <div className="bg-white/[0.04] backdrop-blur-2xl rounded-2xl border border-white/[0.07] shadow-2xl p-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { label: 'Model', value: data.tech_insights.model }, { label: 'Embeddings', value: data.tech_insights.embedding_model },
                      { label: 'Vector DB', value: data.tech_insights.vector_db }, { label: 'Language', value: (data.tech_insights.language || 'en').toUpperCase() },
                      { label: 'Chunks Indexed', value: data.tech_insights.chunks?.toLocaleString() }, { label: 'Tokens', value: data.tech_insights.tokens_processed?.toLocaleString() },
              ...
              <div className="bg-white/[0.04] backdrop-blur-2xl rounded-2xl border border-white/[0.07] shadow-2xl p-5">
                ...
                <div className="space-y-2.5">
                  {[
                    { label: 'Duration', value: formatTime(data.duration) }, { label: 'Chapters', value: data.chapters?.length || 0 },
                    { label: 'Segments', value: data.transcript?.length || 0 }, { label: 'Key Moments', value: data.key_moments?.length || 0 },
              ...
              <div className="flex gap-2">
                <button onClick={() => downloadAs('json')} className="flex-1 px-4 py-2.5 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.07] rounded-xl text-sm text-gray-300">Download JSON</button>
                <button onClick={() => downloadAs('md')} className="flex-1 px-4 py-2.5 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.07] rounded-xl text-sm text-gray-300">Download Markdown</button>
              </div>
            </div>
          </div>
        )}
